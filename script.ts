const diceImage = document.querySelector(".dice") as HTMLImageElement;
const newGameBtn = document.querySelector(".btn--new");
const rollBtn = document.querySelector(".btn--roll");
const holdBtn = document.querySelector(".btn--hold");

let activePlayer = 0;
const player1 = document.querySelector(".player--0") as HTMLElement;
const player2 = document.querySelector(".player--1") as HTMLElement;

function getActivePlayer():HTMLElement {
...
function displayDice(dice: number) {
    diceImage!.classList.remove("hidden");
    diceImage!.src = `images/dice-${String(dice)}.png`;
}

function updateCurrentScore(dice: number) {
    const currentScore = document.getElementById(`current--${activePlayer}`);
    let score: number = Number(currentScore!.textContent);
    score += dice;
...
function updateScore() {
    ...
    let score: number = Number(scorePlayer!.textContent);
    let currentDice: number = Number(currentDicePlayer!.textContent);
    score += currentDice;
